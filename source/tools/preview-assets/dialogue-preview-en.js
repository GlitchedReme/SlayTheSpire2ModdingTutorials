...
(function () {
  ...
  const state = {
    ancientId: 'TEZCATARA',
    ancientName: 'Tezcatara',
    ancientEpithet: 'Fire Keeper',
  ...
  function renderCharSelect() {
    ...
    const opts = ordered.map((c) => {
      const label = c === FIRST_VISIT ? 'First Meeting'
                  : c === ANY_CHAR    ? 'Any Character'
                  : c;
      const count = (state.chars[c] || []).length;
      return '<option value="' + escapeHtml(c) + '"'
        + (c === state.activeChar ? ' selected' : '') + '>'
        + escapeHtml(label) + ' · ' + count + ' sets</option>';
    });
    opts.push('<option value="__add__">＋ Add New Character…</option>');
  ...
  function addNewChar() {
    const name = prompt('New character ID (UPPERCASE, e.g. WATCHER)：');
    if (!name) {
    ...
    if (state.chars[id]) { toast('Already exists: ' + id); renderCharSelect(); return; }
  ...
  function setLabel(s, sIdx, total) {
    return 'Set ' + (sIdx + 1) + ' sets · Set ' + s.visitIndex + '' + (s.repeatable ? ' · 可重复' : '');
  }
  ...
  function renderSetPickerOnly() {
    ...
    if (sets.length === 0) {
      dom.setPicker.innerHTML = '<option value="-1">No dialogue sets</option>';
    } else if (isFirstVisit) {
      dom.setPicker.innerHTML = '<option value="0" selected>First Meeting</option>';
    } else {
  ...
  function renderSets() {
    ...
    if (cur && !isFirstVisit) {
      dom.setFields.innerHTML =
        ...
        '<label class="dp-meta-cell dp-meta-checkbox">' +
          ...
          '<span>Repeatable (r)</span>' +
        '</label>';
    } else if (cur && isFirstVisit) {
      dom.setFields.innerHTML = '<span class="dp-set-fields-note">First meeting only, no visit count</span>';
    } else {
      dom.setFields.innerHTML = '';
    }
    ...
    cur.lines.forEach(function (line, lIdx) {
      ...
      const optA = '<option value="ancient"' + (line.speaker === 'ancient' ? ' selected' : '') + '>Ancient</option>';
      const optC = '<option value="character"' + (line.speaker === 'character' ? ' selected' : '') + '>Character</option>';
      row.innerHTML =
        ...
        '<div class="dp-line-text">' +
          ...
          '<input type="text" class="dp-line-next-input" data-field="next" placeholder="Button text" value="' + escapeHtml(line.next || '') + '">' +
        '</div>' +
        '<div class="dp-line-actions">' +
          '<button type="button" data-action="lineUp" title="Move Up">▲</button>' +
          '<button type="button" data-action="lineDown" title="Move Down">▼</button>' +
          '<button type="button" class="dp-line-remove" data-action="lineRemove" title="Delete">✕</button>' +
        '</div>';
  ...
  function renderAncientSelect() {
    ...
    if (customIds.length > 0) {
      parts.push('<option disabled>── Custom ──</option>');
    ...
    parts.push('<option value="__add__">+ New Ancient</option>');
  ...
  function loadAncient(id) {
    ...
    if (isCustomAncient(id)) {
      const raw = loadCustomData(id);
      if (!raw) { toast('Custom ancient data lost: ' + id); return; }
      state.ancientId = id;
    ...
    if (!filtered || Object.keys(filtered).length === 0) {
      toast('Ancient data not found: ' + id);
      renderAncientSelect();
      return;
    }
  ...
  dom.ancientId.addEventListener('change', () => {
    ...
    if (val === '__add__') {
      const idInput = prompt('New ancient ID (UPPERCASE, e.g. MY_ANCIENT)：');
      ...
      if (ANCIENT_LABELS[newId] || isCustomAncient(newId)) {
        toast('Already exists: ' + newId);
        renderAncientSelect();
        return;
      }
      const nameInput = prompt('Display name (e.g. My Ancient)：');
  ...
  dom.deleteAncient.addEventListener('click', () => {
    if (!state.ancientId || !isCustomAncient(state.ancientId)) return;
    if (!confirm('Delete custom ancient "' + (state.ancientName || state.ancientId) + '"？不可撤销。')) return;
  ...
  dom.deleteChar.addEventListener('click', () => {
    ...
    if (!confirm('DeleteCharacter "' + c + '" 及其所有对话集？不可撤销。')) return;
  ...
  function visualFromJson(silent) {
    ...
    catch (err) { if (!silent) toast('JSON parse failed: ' + err.message); return false; }
    if (!parsed || typeof parsed !== 'object') { if (!silent) toast('JSON must be an object'); return false; }
  ...
  function renderPreview(resetCursor) {
    ...
    if (parts.length === 0) {
      parts.push('<div class="dp-empty-hint">This dialogue set is empty</div>');
    }
    dom.dialogueStack.innerHTML = parts.join('');
    dom.progress.textContent = 'Set ' + state.cursor + ' / ' + total + ' lines';
