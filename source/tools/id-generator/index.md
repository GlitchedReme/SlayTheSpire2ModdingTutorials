...
<span>类名（每行一个）</span>
<span class="entry-id-tool__help" tabindex="0" aria-label="导入说明">
<span class="entry-id-tool__help-mark">?</span>
<span class="entry-id-tool__help-tip" role="tooltip">递归读取文件夹及其所有子文件夹下的 .cs 文件名并追加为类名（自动忽略 .uid / .import 等元数据文件）。文件仅在本地浏览器读取，不会上传。</span>
</span>
<button class="entry-id-tool__copy" id="entryIdImport" type="button">导入文件夹</button>
